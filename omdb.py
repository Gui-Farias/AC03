...
def dicionario_do_filme_por_id(id_filme):
    url = "http://www.omdbapi.com/?i={}&apikey={}".format(id_filme, api_key)
    retorno = req.get(url).json()
    filmin = {'ano': retorno['Year'], 'nome': retorno['Title'], 'diretor' : retorno['Director'], 'genero' : retorno['Genre'], 'poster': retorno['Poster']}
    if filmin=={'Error': 'Incorrect IMDb ID.', 'Response': 'False'}:
        raise Exception()
    return filmin
